...
import diffusers
from diffusers import UNet2DConditionModel, AutoPipelineForText2Image, DDPMScheduler, PriorTransformer, StableDiffusionPipeline
...
def main():
    ...
    def preprocess_train(examples):
        images = [train_transforms(image.convert("RGB")) for image in examples[image_column]]
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("textinversion-fine-tune", config=vars(args))
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(prior):
                ...
                if accelerator.sync_gradients:
                    accelerator.clip_grad_norm_(text_inversion_layers, args.max_grad_norm)
                optimizer.step()
                lr_scheduler.step()
                optimizer.zero_grad()
    ...
    # Save the text inversion layers
    ...
    if accelerator.is_main_process:
        ...
        required_parameters = [p for p in unwrapped_text_encoder.parameters() if p.requires_grad]
        torch.save(required_parameters, os.path.join(args.output_dir, "text_inversion_model.pth"))
