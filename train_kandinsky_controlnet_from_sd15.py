...
def log_validation(image_encoder, movq, unet, controlnet, tokenizer, text_encoder, accelerator, weight_dtype, args):
    ...
    image_logs = []
    for val_prompt, val_cond in zip(args.val_prompts, args.val_conds):
        ...
        image_logs.append(
            {"images": image, "validation_prompts": val_prompt, "validation_control": val_cond}
        )
    ...
    for tracker in accelerator.trackers:
        if tracker.name == "wandb":
            ...
            for log in image_logs:
                ...
                formatted_images.append(wandb.Image(validation_control, caption=validation_prompt))
...
def make_train_dataset(path, image_processor, tokenizer, accelerator, args):
...
def main():
    ...
    with ContextManagers(deepspeed_zero_init_disabled_context_manager()):
        ...
        tokenizer = CLIPTokenizer.from_pretrained(
            args.decoder_model_path, subfolder="tokenizer"
        )
        text_encoder = CLIPTextModel.from_pretrained(
            args.decoder_model_path, subfolder="text_encoder"
        ).eval()

    if args.init == True:
        controlnet = ControlNetModel.from_config(os.path.join(args.decoder_model_path, "controlnet/config.json"))
    else:
        controlnet = ControlNetModel.from_pretrained(args.decoder_model_path, subfolder="controlnet/tile_from_sd15")
